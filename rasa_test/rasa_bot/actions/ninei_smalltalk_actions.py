...
class ActionSmalltalkFirst(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        is_real_time = False
        if smalltalk_step in [2, 4, 10, 21, 22, 25, 29, 32, 33, 34, 35, 37, 38, 43, 44]: # 실시간 생성하는 경우
            vID = get_TTS(question, metadata, voice_create) #생성하고 경로를 가져옴 /// get_TTS의 함수를 고객 이름 + vid로 해야 겠음
